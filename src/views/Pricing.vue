<template>
  <div class="pricing">
    <h1 class="pricing__title">Precios</h1>
    <p class="pricing__description">
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Architecto nihil
      fugit suscipit necessitatibus ipsum inventore?
    </p>
    <div class="pricing__prices">
      <div class="pricing__prices__price pricing__prices__price--blue">
        <div class="pricing__prices__price__details">
          <h2 class="pricing__prices__price__details__title">Starter</h2>
          <div class="pricing__prices__price__details__price">
            <span class="pricing__prices__price__details__price__currency"
              >$</span
            >50
            <div class="pricing__prices__price__details__month">
              <span class="pricing__prices__price__details__month__off">
                300
              </span>
              <span class="pricing__prices__price__details__month__price">
                USD/año
              </span>
            </div>
          </div>
          <span class="pricing__prices__price__details__year">Per year</span>
        </div>
        <p class="pricing__prices__price__description">
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem
          aliquid ipsa quae, ratione doloremque aliquam?
        </p>
        <div class="pricing__prices__price__features">
          <h3 class="pricing__prices__price__features__title">
            Lorem ipsum dolor sit.
          </h3>

          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--blue"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--blue"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--blue"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--blue"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
        </div>
      </div>
      <div
        class="pricing__prices__price pricing__prices__price--orange pricing__prices__price--middle pricing__prices__price--middle2"
      >
        <div class="pricing__prices__price__details">
          <h2 class="pricing__prices__price__details__title">Starter</h2>
          <div class="pricing__prices__price__details__price">
            <span class="pricing__prices__price__details__price__currency"
              >$</span
            >50
            <div class="pricing__prices__price__details__month">
              <span class="pricing__prices__price__details__month__off">
                300
              </span>
              <span class="pricing__prices__price__details__month__price">
                USD/año
              </span>
            </div>
          </div>
          <span class="pricing__prices__price__details__year">Per year</span>
        </div>
        <p class="pricing__prices__price__description">
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem
          aliquid ipsa quae, ratione doloremque aliquam?
        </p>
        <div class="pricing__prices__price__features">
          <h3 class="pricing__prices__price__features__title">
            Lorem ipsum dolor sit.
          </h3>

          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
        </div>
      </div>
      <div
        class="pricing__prices__price pricing__prices__price--orange pricing__prices__price--third"
      >
        <div class="pricing__prices__price__details">
          <h2 class="pricing__prices__price__details__title">Starter</h2>
          <div class="pricing__prices__price__details__price">
            <span class="pricing__prices__price__details__price__currency"
              >$</span
            >50
            <div class="pricing__prices__price__details__month">
              <span class="pricing__prices__price__details__month__off">
                300
              </span>
              <span class="pricing__prices__price__details__month__price">
                USD/año
              </span>
            </div>
          </div>
          <span class="pricing__prices__price__details__year">Per year</span>
        </div>
        <p class="pricing__prices__price__description">
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem
          aliquid ipsa quae, ratione doloremque aliquam?
        </p>
        <div class="pricing__prices__price__features">
          <h3 class="pricing__prices__price__features__title">
            Lorem ipsum dolor sit.
          </h3>

          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
          <p class="pricing__prices__price__features__feature">
            <span
              class="pricing__prices__price__features__feature__bubble pricing__prices__price__features__feature__bubble--orange"
            ></span>
            Lorem ipsum dolor sit amet.
          </p>
        </div>
      </div>
    </div>
    <div class="pricing__plans">
      <section class="table-pricing">
        <h1 class="table-pricing__title">Comparar Planes</h1>

        <div class="table-pricing__header">
          <div
            class="table-pricing__header-container table-pricing__header-container-empty "
          ></div>
          <div
            class="table-pricing__header-container table-pricing__header-container-bewe"
          >
            <svg
              id="Grupo_32"
              data-name="Grupo 32"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              width="110.395"
              height="30.634"
              viewBox="0 0 110.395 30.634"
            >
              <defs>
                <clipPath id="clip-path">
                  <rect
                    id="Rectángulo_8"
                    data-name="Rectángulo 8"
                    width="110.395"
                    height="30.634"
                    fill="none"
                  />
                </clipPath>
              </defs>
              <g id="Grupo_3" data-name="Grupo 3" clip-path="url(#clip-path)">
                <path
                  id="Trazado_6"
                  data-name="Trazado 6"
                  d="M0,2.408A2.318,2.318,0,0,1,.708.709,2.318,2.318,0,0,1,2.407,0a2.317,2.317,0,0,1,1.7.708,2.318,2.318,0,0,1,.708,1.7V9.347a11.291,11.291,0,0,1,6.962-2.266,11.35,11.35,0,0,1,8.331,3.446,11.349,11.349,0,0,1,3.446,8.331,11.349,11.349,0,0,1-3.446,8.331,11.348,11.348,0,0,1-8.331,3.446,11.347,11.347,0,0,1-8.331-3.446A11.356,11.356,0,0,1,0,18.976ZM6.821,23.814a6.754,6.754,0,0,0,4.956,2.053,6.733,6.733,0,0,0,4.944-2.042,6.734,6.734,0,0,0,2.042-4.944,6.774,6.774,0,0,0-2.042-4.956,6.714,6.714,0,0,0-4.944-2.053,6.753,6.753,0,0,0-4.956,2.053,6.7,6.7,0,0,0-2.006,4.107v.9a3.916,3.916,0,0,1-.024.425,6.581,6.581,0,0,0,2.03,4.461"
                  transform="translate(0 -0.001)"
                  fill="#0097a6"
                />
                <path
                  id="Trazado_7"
                  data-name="Trazado 7"
                  d="M138.34,39.749a7.093,7.093,0,0,0-7.812,1.451,7.005,7.005,0,0,0,9.889,9.924l.047-.047a2.278,2.278,0,0,1,1.675-.685,2.338,2.338,0,0,1,1.7.7,2.277,2.277,0,0,1,.708,1.688,2.315,2.315,0,0,1-.708,1.7l-.047.047a11.777,11.777,0,0,1-20.085-8.355,11.782,11.782,0,0,1,20.085-8.355l-.024.023a.021.021,0,0,0,.024.024,2.281,2.281,0,0,1,.684,1.676,2.252,2.252,0,0,1-.684,1.652l-6.75,6.7a2.357,2.357,0,0,1-1.7.685,2.34,2.34,0,0,1-2.407-2.407,2.357,2.357,0,0,1,.684-1.7Z"
                  transform="translate(-98.24 -27.312)"
                  fill="#0097a6"
                />
                <path
                  id="Trazado_8"
                  data-name="Trazado 8"
                  d="M449.634,39.749a7.093,7.093,0,0,0-7.812,1.451,7.005,7.005,0,0,0,9.889,9.924l.047-.047a2.278,2.278,0,0,1,1.675-.685,2.338,2.338,0,0,1,1.7.7,2.278,2.278,0,0,1,.708,1.688,2.316,2.316,0,0,1-.708,1.7l-.047.047A11.777,11.777,0,0,1,435,46.169a11.782,11.782,0,0,1,20.085-8.355l-.024.023a.021.021,0,0,0,.024.024,2.281,2.281,0,0,1,.684,1.676,2.252,2.252,0,0,1-.684,1.652l-6.75,6.7a2.358,2.358,0,0,1-1.7.685,2.34,2.34,0,0,1-2.408-2.407,2.357,2.357,0,0,1,.684-1.7Z"
                  transform="translate(-345.446 -27.312)"
                  fill="#002239"
                />
                <path
                  id="Trazado_9"
                  data-name="Trazado 9"
                  d="M251.456,34.162a2.279,2.279,0,0,1,1.676.685,2.2,2.2,0,0,1,.378.5h.023l7.363,14.75,7.364-14.75.023.023a2.539,2.539,0,0,1,.4-.519,2.35,2.35,0,0,1,3.328.012,2.289,2.289,0,0,1,.685,1.664,2.2,2.2,0,0,1-.448,1.369l-9.134,18.338a2.043,2.043,0,0,1-.543.826,2.241,2.241,0,0,1-1.676.708,2.215,2.215,0,0,1-1.652-.708,2.082,2.082,0,0,1-.543-.8l-7.246-14.515-7.222,14.491a2.044,2.044,0,0,1-.543.826,2.242,2.242,0,0,1-1.676.708,2.213,2.213,0,0,1-1.652-.708,2.075,2.075,0,0,1-.543-.8L230.64,37.867a2.24,2.24,0,0,1-.425-1.345,2.37,2.37,0,0,1,4.036-1.676,2.189,2.189,0,0,1,.377.5h.024l7.363,14.75,7.364-14.75.023.023a2.54,2.54,0,0,1,.4-.519,2.25,2.25,0,0,1,1.652-.685"
                  transform="translate(-182.82 -27.129)"
                  fill="#002239"
                />
              </g>
            </svg>

            <a class="table-pricing__button table-pricing__button-bewe" href="">
              readable
            </a>
          </div>
          <div
            class="table-pricing__header-container table-pricing__header-container-bewe-one"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="79.231"
              height="25.704"
              viewBox="0 0 79.231 25.704"
            >
              <g
                id="Grupo_33"
                data-name="Grupo 33"
                transform="translate(-66.147 -16.148)"
              >
                <g id="Grupo_34" data-name="Grupo 34">
                  <path
                    id="Trazado_12"
                    data-name="Trazado 12"
                    d="M438.78,105.846a12.846,12.846,0,1,1-9.074-3.753,12.363,12.363,0,0,1,9.074,3.753m-9.074,1.439a7.591,7.591,0,1,0,5.385,2.223,7.378,7.378,0,0,0-5.385-2.223"
                    transform="translate(-350.733 -85.894)"
                    fill="#ff9a5d"
                  />
                  <path
                    id="Trazado_13"
                    data-name="Trazado 13"
                    d="M608.111,101.769a13.2,13.2,0,0,1,8.919,3.8,12.369,12.369,0,0,1,3.753,8.945v9.922a3.065,3.065,0,0,1-.771,2.262,2.607,2.607,0,0,1-3.7,0,2.991,2.991,0,0,1-.771-2.211V114.57a4.168,4.168,0,0,1,.026-.463,7.168,7.168,0,0,0-2.211-4.858,7.356,7.356,0,0,0-5.4-2.236h-.1a7.355,7.355,0,0,0-5.4,2.236,7.167,7.167,0,0,0-2.211,4.858,4.259,4.259,0,0,1,.026.463v9.922a2.991,2.991,0,0,1-.771,2.211,2.606,2.606,0,0,1-3.7,0,3.065,3.065,0,0,1-.771-2.262v-9.922a12.368,12.368,0,0,1,3.753-8.945,13.2,13.2,0,0,1,8.919-3.8Z"
                    transform="translate(-500.613 -85.621)"
                    fill="#ff9a5d"
                  />
                  <path
                    id="Trazado_14"
                    data-name="Trazado 14"
                    d="M789.112,107.928a7.725,7.725,0,0,0-8.508,1.581,7.629,7.629,0,0,0,10.77,10.809l.051-.051a2.483,2.483,0,0,1,1.825-.745,2.546,2.546,0,0,1,1.851.758,2.482,2.482,0,0,1,.771,1.838,2.524,2.524,0,0,1-.771,1.851l-.051.051a12.826,12.826,0,1,1,0-18.2l-.026.026a.023.023,0,0,0,.026.026,2.483,2.483,0,0,1,.745,1.825,2.452,2.452,0,0,1-.745,1.8l-7.352,7.3a2.567,2.567,0,0,1-1.851.745,2.549,2.549,0,0,1-2.622-2.622,2.568,2.568,0,0,1,.746-1.851Z"
                    transform="translate(-650.494 -85.894)"
                    fill="#ff9a5d"
                  />
                </g>
              </g>
            </svg>

            <a
              class="table-pricing__button table-pricing__button-bewe-one"
              href=""
            >
              readable
            </a>
          </div>

          <div
            class="table-pricing__header-container table-pricing__header-container-bewe-one-plus"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="107.168"
              height="25.704"
              viewBox="0 0 107.168 25.704"
            >
              <g
                id="Grupo_35"
                data-name="Grupo 35"
                transform="translate(-66.147 -16.148)"
              >
                <path
                  id="Trazado_1594"
                  data-name="Trazado 1594"
                  d="M438.78,105.846a12.846,12.846,0,1,1-9.074-3.753,12.363,12.363,0,0,1,9.074,3.753m-9.074,1.439a7.591,7.591,0,1,0,5.385,2.223,7.378,7.378,0,0,0-5.385-2.223"
                  transform="translate(-350.733 -85.894)"
                  fill="#ff9a5d"
                />
                <path
                  id="Trazado_1595"
                  data-name="Trazado 1595"
                  d="M608.11,101.769a13.2,13.2,0,0,1,8.92,3.8,12.37,12.37,0,0,1,3.753,8.945v9.922a3.065,3.065,0,0,1-.771,2.262,2.607,2.607,0,0,1-3.7,0,2.991,2.991,0,0,1-.771-2.211V114.57a4.139,4.139,0,0,1,.026-.463,7.168,7.168,0,0,0-2.211-4.858,7.356,7.356,0,0,0-5.4-2.236h-.1a7.356,7.356,0,0,0-5.4,2.236,7.167,7.167,0,0,0-2.211,4.858,4.259,4.259,0,0,1,.026.463v9.922a2.991,2.991,0,0,1-.771,2.211,2.607,2.607,0,0,1-3.7,0,3.066,3.066,0,0,1-.771-2.262v-9.922a12.368,12.368,0,0,1,3.753-8.945,13.2,13.2,0,0,1,8.919-3.8Z"
                  transform="translate(-500.613 -85.621)"
                  fill="#ff9a5d"
                />
                <path
                  id="Trazado_1596"
                  data-name="Trazado 1596"
                  d="M789.112,107.928a7.725,7.725,0,0,0-8.508,1.581,7.629,7.629,0,0,0,10.77,10.809l.051-.051a2.483,2.483,0,0,1,1.825-.745,2.546,2.546,0,0,1,1.851.758,2.482,2.482,0,0,1,.771,1.838,2.524,2.524,0,0,1-.771,1.851l-.051.051a12.826,12.826,0,1,1,0-18.2l-.026.026a.023.023,0,0,0,.026.026,2.483,2.483,0,0,1,.745,1.825,2.452,2.452,0,0,1-.745,1.8l-7.352,7.3a2.567,2.567,0,0,1-1.851.745,2.549,2.549,0,0,1-2.622-2.622,2.568,2.568,0,0,1,.745-1.851Z"
                  transform="translate(-650.494 -85.894)"
                  fill="#ff9a5d"
                />
                <path
                  id="Trazado_1597"
                  data-name="Trazado 1597"
                  d="M968.021,131.108h-3.377a3.369,3.369,0,0,1-2.5-1.033,3.536,3.536,0,0,1,0-4.962,3.367,3.367,0,0,1,2.5-1.034h3.377V120.7a3.41,3.41,0,0,1,1.016-2.481,3.523,3.523,0,0,1,6.012,2.481v3.377h3.377a3.514,3.514,0,0,1,0,7.029H975.05v3.377a3.514,3.514,0,1,1-7.029,0Z"
                  transform="translate(-808.626 -98.594)"
                  fill="#ff9a5d"
                />
              </g>
            </svg>

            <a
              href=""
              class="table-pricing__button table-pricing__button-bewe-one-plus"
            >
              readable
            </a>
          </div>
        </div>

        <div class="table-pricing__body">
          <h2 class="table-pricing__body-title">
            Recopile y envie datos a cualquier lugar
          </h2>
          <div
            @click="toogleSection('table-pricing__body-row-one')"
            id="table-pricing__body-row-one"
            class="table-pricing__body-row"
          >
            <div class="table-pricing__body-row-first">
              <div class="table-pricing__body-row-first-arounds">
                <svg
                  v-if="!visibleSectionOneChild"
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                  fill="#000000"
                >
                  <path d="M0 0h24v24H0V0z" fill="none" />
                  <path
                    d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6-6-6z"
                  />
                </svg>

                <svg
                  class="d-none"
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                  fill="#000000"
                >
                  <path d="M24 24H0V0h24v24z" fill="none" opacity=".87" />
                  <path
                    d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6-1.41-1.41z"
                  />
                </svg>
              </div>
              <p class="table-pricing__body-row-first-text">Established</p>

              <svg
                xmlns="http://www.w3.org/2000/svg"
                height="24px"
                viewBox="0 0 24 24"
                width="24px"
                fill="#000000"
              >
                <path d="M0 0h24v24H0V0z" fill="none" />
                <path
                  d="M11 7h2v2h-2zm0 4h2v6h-2zm1-9C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"
                />
              </svg>
            </div>

            <div>
              <p>Established</p>
            </div>
            <div class="table-pricing__body-row-medium border-x">
              <p>
                established
              </p>
            </div>
            <div>
              <p>Established</p>
            </div>
          </div>

          <div
            id="table-pricing__body-row-one-children"
            class="visible table-pricing__body-row-show"
          >
            <div
              class="table-pricing__body-row-show-children table-pricing__body-row-orange"
            >
              <div class="table-pricing__body-row-show-children-first">
                <p class="table-pricing__body-row-first-text">Established</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price border-x">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>
            </div>

            <div class="table-pricing__body-row-show-children">
              <div class="table-pricing__body-row-show-children-first">
                <p class="table-pricing__body-row-first-text">Established</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price border-x">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>
            </div>
          </div>

          <div
            @click="toogleSection('table-pricing__body-row-two')"
            id="table-pricing__body-row-two"
            class="table-pricing__body-row table-pricing__body-row-orange"
          >
            <div class="table-pricing__body-row-first">
              <div class="table-pricing__body-row-first-arounds">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                  fill="#000000"
                >
                  <path d="M0 0h24v24H0V0z" fill="none" />
                  <path
                    d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6-6-6z"
                  />
                </svg>

                <svg
                  class="d-none"
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                  fill="#000000"
                >
                  <path d="M24 24H0V0h24v24z" fill="none" opacity=".87" />
                  <path
                    d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6-1.41-1.41z"
                  />
                </svg>
              </div>
              <p class="table-pricing__body-row-first-text">Established</p>

              <svg
                xmlns="http://www.w3.org/2000/svg"
                height="24px"
                viewBox="0 0 24 24"
                width="24px"
                fill="#000000"
              >
                <path d="M0 0h24v24H0V0z" fill="none" />
                <path
                  d="M11 7h2v2h-2zm0 4h2v6h-2zm1-9C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"
                />
              </svg>
            </div>

            <div>
              <p>Established</p>
            </div>
            <div class="border-x">
              <p>
                established
              </p>
            </div>
            <div>
              <p>Established</p>
            </div>
          </div>

          <div
            id="table-pricing__body-row-two-children"
            class="visible table-pricing__body-row-show"
          >
            <div class="table-pricing__body-row-show-children">
              <div class="table-pricing__body-row-show-children-first">
                <p class="table-pricing__body-row-first-text">Established</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price border-x">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>
            </div>

            <div
              class="table-pricing__body-row-show-children table-pricing__body-row-orange"
            >
              <div class="table-pricing__body-row-show-children-first">
                <p class="table-pricing__body-row-first-text">Established</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price border-x">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>
            </div>

            <div class="table-pricing__body-row-show-children">
              <div class="table-pricing__body-row-show-children-first">
                <p class="table-pricing__body-row-first-text">Established</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price border-x">
                <p>free</p>
              </div>

              <div class="table-pricing__body-row-show-children-price">
                <p>free</p>
              </div>
            </div>
          </div>

          <div
            @click="toogleSection('table-pricing__body-row-three')"
            id="table-pricing__body-row-three"
            class="table-pricing__body-row"
          >
            <div class="table-pricing__body-row-first">
              <div class="table-pricing__body-row-first-arounds">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                  fill="#000000"
                >
                  <path d="M0 0h24v24H0V0z" fill="none" />
                  <path
                    d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6-6-6z"
                  />
                </svg>

                <svg
                  class="d-none"
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                  fill="#000000"
                >
                  <path d="M24 24H0V0h24v24z" fill="none" opacity=".87" />
                  <path
                    d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6-1.41-1.41z"
                  />
                </svg>
              </div>
              <p class="table-pricing__body-row-first-text">Established</p>

              <svg
                xmlns="http://www.w3.org/2000/svg"
                height="24px"
                viewBox="0 0 24 24"
                width="24px"
                fill="#000000"
              >
                <path d="M0 0h24v24H0V0z" fill="none" />
                <path
                  d="M11 7h2v2h-2zm0 4h2v6h-2zm1-9C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"
                />
              </svg>
            </div>

            <div>
              <p>Established</p>
            </div>
            <div class="border-x">
              <p>
                established
              </p>
            </div>
            <div>
              <p>Established</p>
            </div>
          </div>
        </div>
      </section>
    </div>
    <div class="pricing__faq--container">
      <div class="pricing__faq">
        <h2 class="pricing__faq__title">Preguntas Frecuentes</h2>

        <div class="pricing__faq__faqs">
          <div @click="($event) => openFAQ($event)" class="pricing__faq__faqs__faq">
            <div class="pricing__faq__faqs__faq__title">
              Aqui Lorem ipsum, dolor sit amet consectetur adipisicing elit. In, modi
              quis nulla accusamus natus dolore?
            </div>
            <p class="pricing__faq__faqs__faq__text d-none">
              Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sequi,
              temporibus itaque beatae obcaecati amet asperiores exercitationem
              ipsum debitis alias. Dolore laudantium eos ipsa assumenda
              consequatur corporis ducimus enim iste omnis.
            </p>
            <span class="pricing__faq__faqs__faq__arrow">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="{2}"
                  d="M9 5l7 7-7 7"
                />
              </svg>
            </span>
          </div>
          <div @click="($event) => openFAQ($event)" class="pricing__faq__faqs__faq">
            <div class="pricing__faq__faqs__faq__title">
              Aqui Lorem ipsum, dolor sit amet consectetur adipisicing elit. In, modi
              quis nulla accusamus natus dolore?
            </div>
            <p class="pricing__faq__faqs__faq__text d-none">
              Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sequi,
              temporibus itaque beatae obcaecati amet asperiores exercitationem
              ipsum debitis alias. Dolore laudantium eos ipsa assumenda
              consequatur corporis ducimus enim iste omnis.
            </p>
            <span class="pricing__faq__faqs__faq__arrow">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="{2}"
                  d="M9 5l7 7-7 7"
                />
              </svg>
            </span>
          </div>
          <div @click="($event) => openFAQ($event)" class="pricing__faq__faqs__faq">
            <div class="pricing__faq__faqs__faq__title">
              Aqui Lorem ipsum, dolor sit amet consectetur adipisicing elit. In, modi
              quis nulla accusamus natus dolore?
            </div>
            <p class="pricing__faq__faqs__faq__text d-none">
              Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sequi,
              temporibus itaque beatae obcaecati amet asperiores exercitationem
              ipsum debitis alias. Dolore laudantium eos ipsa assumenda
              consequatur corporis ducimus enim iste omnis.
            </p>
            <span class="pricing__faq__faqs__faq__arrow">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="{2}"
                  d="M9 5l7 7-7 7"
                />
              </svg>
            </span>
          </div>
          <div @click="($event) => openFAQ($event)" class="pricing__faq__faqs__faq">
            <div class="pricing__faq__faqs__faq__title">
              Aqui Lorem ipsum, dolor sit amet consectetur adipisicing elit. In, modi
              quis nulla accusamus natus dolore?
            </div>
            <p class="pricing__faq__faqs__faq__text d-none">
              Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sequi,
              temporibus itaque beatae obcaecati amet asperiores exercitationem
              ipsum debitis alias. Dolore laudantium eos ipsa assumenda
              consequatur corporis ducimus enim iste omnis.
            </p>
            <span class="pricing__faq__faqs__faq__arrow">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="{2}"
                  d="M9 5l7 7-7 7"
                />
              </svg>
            </span>
          </div>
          <div @click="($event) => openFAQ($event)" class="pricing__faq__faqs__faq">
            <div class="pricing__faq__faqs__faq__title">
              Aqui Lorem ipsum, dolor sit amet consectetur adipisicing elit. In, modi
              quis nulla accusamus natus dolore?
            </div>
            <p class="pricing__faq__faqs__faq__text d-none">
              Lorem ipsum dolor sit, amet consectetur adipisicing elit. Sequi,
              temporibus itaque beatae obcaecati amet asperiores exercitationem
              ipsum debitis alias. Dolore laudantium eos ipsa assumenda
              consequatur corporis ducimus enim iste omnis.
            </p>
            <span class="pricing__faq__faqs__faq__arrow">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="{2}"
                  d="M9 5l7 7-7 7"
                />
              </svg>
            </span>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
    name: 'TablePricing',
    data(){
        return{
            visibleSectionOneChild: false,
        }
    }, methods: {
 
        toogleSection(id){
            // const elementClick = document.getElementById(id);
            const elementShow = document.getElementById(`${id}-children`);
            const elementsArounds = document.querySelectorAll(`#${id} div div svg`);
 
            elementsArounds.forEach( e => e.classList?.toggle('d-none')); 
            elementShow.classList?.toggle('visible');
 
        },

        openFAQ(e){
            const element = e.target;
            element.children[1].classList.toggle("d-none");
        }
    }
 
}
</script>

<style scoped>
/* Mobile 320px */
.pricing {
  width: 100%;
  padding: 0px 10px;
  text-align: center;
}

.pricing__title {
  color: #141a10;
  font-weight: 700;
  font-size: 25px;
  margin: 0;
  padding: 0;
}

.pricing__description {
  color: gray;
  font-size: 16px;
}

.pricing__prices {
  align-items: center;
  display: flex;
  flex-direction: column;
  justify-content: center;
}

.pricing__prices__price {
  border-radius: 10px;
  margin-bottom: 30px;
  padding: 10px 5px;
  position: relative;
  width: 100%;
}

.pricing__prices__price--blue {
  border: 2px solid #22a5b2;
}

.pricing__prices__price--middle {
  position: relative;
  height: 600px !important;
}

.pricing__prices__price--middle::before {
  content: "";
  background-color: #ff9a5d;
  height: 20px;
  width: 120px;
  border-top-left-radius: 10px;
  border-top-right-radius: 10px;
  top: -20px;
  left: 10px;
  position: absolute;
}

.pricing__prices__price--middle::after {
  content: "";
  background-image: url(https://i.imgur.com/jLCT5DV.png);
  background-size: 60%;
  top: -42px;
  right: -50px;
  width: 150px;
  height: 60px;
  display: block;
  background-repeat: no-repeat;
  position: absolute;
}

.pricing__prices__price--third::before {
  content: "";
  background-image: url(https://i.imgur.com/PRdFb2l.png);
  background-size: 60%;
  bottom: 0px;
  right: -136px;
  width: 200px;
  height: 105px;
  display: block;
  background-repeat: no-repeat;
  position: absolute;
}

.pricing__prices__price--third::after {
  content: "";
  background-image: url(https://i.imgur.com/0yBeHPr.png);
  background-size: 60%;
  top: -37px;
  right: -75px;
  width: 200px;
  height: 60px;
  display: block;
  background-repeat: no-repeat;
  position: absolute;
}

.pricing__prices__price--orange {
  border: 2px solid #ff9a5d;
}

.pricing__prices__price__details,
.pricing__prices__price__description {
  /* border-bottom: 1px solid #ccc; */
  /* padding: 15px; */
}

.pricing__prices__price__details__title {
  color: #141a10;
  margin: 0;
  padding: 0;
  font-weight: 400;
  font-size: 28px;
}

.pricing__prices__price__details__price {
  align-items: center;
  display: flex;
  color: #141a10;
  font-weight: 700;
  font-size: 28px;
  justify-content: center;
  margin: 0;
  padding: 0;
}

.pricing__prices__price__details__price__currency {
  color: #141a10;
  font-weight: 700;
  font-size: 16px;
}

.pricing__prices__price__details__month {
  display: flex;
  flex-direction: column;
  font-size: 14px;
  font-weight: 700;
  margin-left: 10px;
  position: relative;
}

.pricing__prices__price__details__month__off::before {
  background-color: #ff9a5d;
  content: "";
  display: block;
  height: 30px;
  left: 50%;
  top: -5px;
  position: absolute;
  transform: rotate(60deg) translateX(-50%);
  width: 2px;
}

.pricing__prices__price__details__year {
  color: gray;
  font-size: 14px;
}

.pricing__prices__price__description {
  border-bottom: 1px solid #ccc;
  border-top: 1px solid #ccc;
  font-size: 14px;
  margin: 0 auto !important;
  padding: 15px 0;
  width: 85%;
}

.pricing__prices__price__features {
  margin: 0;
  margin-top: 15px;
  padding: 15px;
}

.pricing__prices__price__features__title {
  font-size: 18px;
  font-weight: 500;
  margin: 0;
  margin-bottom: 15px;
  padding: 0;
  text-align: left;
}

.pricing__prices__price__features__feature {
  font-size: 13px;
  margin: 0;
  margin-bottom: 20px;
  padding: 0;
  text-align: left;
}

.pricing__prices__price__features__feature__bubble {
  display: inline-block;
  border-radius: 50%;
  background-color: black;
  height: 10px;
  margin-right: 2px;
  width: 10px;
}

.pricing__prices__price__features__feature__bubble--blue {
  background-color: #22a5b2;
}

.pricing__prices__price__features__feature__bubble--orange {
  background-color: #ff9a5d;
}

/* 768 */
@media (min-width: 768px) {
  .pricing {
    padding: 0px;
  }

  .pricing__title {
    font-size: 35px;
  }

  .pricing__description {
    font-size: 18px;
    margin-bottom: 50px;
  }

  .pricing__prices {
    align-items: flex-end;
    display: flex;
    flex-direction: row;
    justify-content: center;

    flex-basis: calc(100% / 3 - 20px);
  }

  .pricing__prices__price {
    margin: 0px 10px;
  }

  .pricing__prices__price__details {
    margin-bottom: 15px;
  }

  .pricing__prices__price__details__title {
    font-size: 30px;
  }

  .pricing__prices__price__details__price {
    font-size: 50px;
  }

  .pricing__prices__price__details__price__currency {
    font-size: 20px;
  }
}

/* 1200px */
@media (min-width: 1000px) {
  .pricing__prices {
    margin: 0 auto;
    width: 1000px;
  }
}

/* Comparar Planes */
.pricing__plans {
  width: 100%;
  margin: 80px 0;
}

.pricing__plans__title {
  font-size: 30px;
  font-weight: 700;
}

.pricing__plans__table {
  width: 100%;
}

.pricing__plans__table__head {
}

.pricing__plans__table__head__title {
  text-align: left;
}

.pricing__plans__table__head__parent__children {
  display: none;
}

.pricing__plans__table__head__plans {
  display: flex;
}

.pricing__plans__table__head__plans__plan {
  display: flex;
  flex-basis: calc(100% / 3);
  justify-content: center;
}

.pricing__plans__table__head__plans__plan__image {
}

.pricing__plans__table__head__plans__plan__button {
  border-radius: 5px;
  font-size: 16px;
  padding: 5px 15px;
}

.pricing__plans__table__head__plans__plan__button--outline--blue {
  background-color: transparent;
  border: 2px solid #22a5b2;
  color: #22a5b2;
}

.pricing__plans__table__head__plans__plan__button--orange {
  background-color: #ff9a5d;
  color: #ffffff;
}

.pricing__plans__table__head__plans__plan__button--outline--orange {
  background-color: transparent;
  border: 2px solid #ff9a5d;
  color: #ff9a5d;
}

.pricing__plans__table__body {
}

.pricing__plans__table__body__row {
  display: block;
  width: 100%;
}

.pricing__plans__table__body__row__parent {
  display: block;
  text-align: left;
  width: 100%;
}

.pricing__plans__table__body__row__parent__title {
  font-size: 18px;
  font-weight: 700;
  margin: 0;
  padding: 0;
  text-align: left;
}

.pricing__plans__table__body__row__parent__text {
  color: gray;
  font-size: 14px;
  margin: 0;
  padding: 0;
  text-align: left;
}

.pricing__plans__table__body__row__children {
  display: flex;
  margin: 15px 0;
  width: 100%;
}

.pricing__plans__table__body__row__children__child {
  width: calc(100% / 3);
}

.pricing__plans__table__body__row__menu {
  display: none;
}

@media (min-width: 768px) {
  .pricing__plans__table__head__parent {
    display: flex;
  }

  .pricing__plans__table__head__plans {
    align-self: flex-end;
    display: flex;
    width: 60%;
  }

  .pricing__plans__table__head__plans__plan {
    width: calc(100% / 3);
  }
}

/* Preguntas Frecuentes */
.pricing__faq--container {
  background-color: #fbe8d9;
  padding: 30px 15px;
}

.pricing__faq__title {
  color: #141a10;
  font-size: 35px;
  font-weight: 700;
  margin-bottom: 30px;
}

.pricing__faq__faqs {
}

.pricing__faq__faqs__faq {
  background-color: #ffffff;
  border-radius: 10px;
  cursor: pointer;
  margin-bottom: 20px;
  padding: 30px 80px 30px 50px;;
  position: relative;
}

.pricing__faq__faqs__faq::before{
    content: '';
    position: absolute;
    top: 0;
    right: 0;
    bottom: 0;
    left: 0;
}

.pricing__faq__faqs__faq__title {
  color: #141a10;
  font-size: 22px;
  font-weight: 500;
  text-align: left;
}

.pricing__faq__faqs__faq__text {
  text-align: left;
}

.pricing__faq__faqs__faq__arrow {
  position: absolute;
  right: 50px;
  top: 40%;
}

.pricing__faq__faqs__faq__arrow svg {
  stroke: #725ba2;
  stroke-width: 4px;
  width: 25px;
}

@media (min-width: 1200px) {
  .pricing__faq {
    margin: 0 auto;
    width: 1200px;
  }
}
 
figure{
    margin: 0;
}
 
.border-x{
    border-left: 1px solid #FF9A5D;
    border-right: 1px solid #FF9A5D;
}
 
.table-pricing{
    width: 100%;
    
    font-weight: 900; 
}
 
.table-pricing__title{
    text-align: center;
    font-size: 35px;
    font-weight: 700;
}
 
.table-pricing__header{
    max-width: 90%;
    display: grid;
    grid-template-columns: repeat(3, 1fr);
    margin: 0 auto;
 
}
 
.table-pricing__header-container{
    display: flex;
    flex-direction: column;
    justify-content: space-between;
    align-items: center;
    height: 80px;
}
 
.table-pricing__header-container-empty{
    grid-area: 1 / 1;
    display: none;
}
 
.table-pricing__header-container > svg {
    max-width: 50%;
}
 
.table-pricing__button{
    background-color: var(--white);
    max-width: 80%;
    padding: 5px 10px;
    text-decoration: none;
    font-size: 16px;
    font-weight: 600;
}
 
.table-pricing__button-bewe{
    border: 1px solid #0097a6;
    color: #0097a6;
}
 
.table-pricing__button-bewe-one{
    background-color: #FF9A5D;
    border: 1px solid #ffffff;
    color: #ffffff;
}
 
.table-pricing__button-bewe-one-plus{
    border: 1px solid #FF9A5D;
    color: #FF9A5D;
}
 
.table-pricing__header-container-bewe-one{
    border-left: 1px solid #FF9A5D ;
    border-right: 1px solid #FF9A5D;
 
}
 
.table-pricing__body{
    max-width: 90%;
    margin: 0 auto; 
}
 
.table-pricing__body-title{
    font-size: 28px;
    font-weight: 700;
}
 
.table-pricing__body-row{
    display: grid;
    grid-template-columns: repeat(3, 1fr);
    font-weight: 700;
}
 
.table-pricing__body-row{
    text-align: center;
    color: #808080;
}
 
.table-pricing__body-row-first{
    display: flex;
    grid-column: 1 / -1;
    color: #000000 ;
    align-items: center;
}
 
.table-pricing__body-row-show-children-first{
    display: flex;
    grid-column: 1 / -1;
    color: #000000;
}
 
.table-pricing__body-row-medium{
    border-right: 1px solid #FF9A5D;
    border-left: 1px solid #FF9A5D;
}
 
.table-pricing__body-row-orange{
    background-color: #FFF1E8;
}
 
.table-pricing__body-row-first-arounds > svg{
    max-width: 20px;
    margin: 0 10px 0 0;
}
 
.table-pricing__body-row-show-children{
    display: grid;
    grid-template-columns: repeat(3 , 1fr);
    grid-column: 1 / 3;
    text-align: center;
    transition: 1s all ease-in-out;
    pointer-events: none;
}
 
 
 
 
 
.heightActive{
    display: grid;
}
 
.table-pricing__body-row-first-text{
    font-weight: 700; 
    margin:16px;   
}
 
.visible{
    height: 0 !important;
    opacity: 0;
    pointer-events: none;
}
 
@media screen and (min-width: 768px){
 
    .table-pricing__header{
        grid-template-columns: repeat(6, 1fr);
    }
    .table-pricing__header-container-empty{
        display: flex;
        grid-column: 1 / -4;
    }
 
    .table-pricing__body-row{
        grid-template-columns: repeat(6, 1fr);
    }
 
    .table-pricing__body-row-first{
        grid-column: 1 / 4;
    }
 
    .table-pricing__body-row-show-children{
        grid-template-columns: repeat(6, 1fr);
    }
    .table-pricing__body-row-show-children-first{
        justify-content: center;
        grid-column: 1 / 4;
    }
}
</style>
